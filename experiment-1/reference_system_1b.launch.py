...
"""Launch file for system 1 for the reference_system experiment with one multithreaded process."""
...
def get_launch_description(group_id: int, group_num_nodes: int) -> launch.LaunchDescription:
    ...
    def get_executor_type(group_id: int, node_id: str) -> str:
        # Use multithreaded executor for one of the executables (see reference system)
        if f'{group_id}{node_id}' == '1b':
    ...
    nodes = [
        launch_ros.actions.Node(
            ...
            executable=(
                f'autoware_default_{get_executor_type(group_id, node_id)}_{group_id}{node_id}'
            ),
        )
    ...
    return launch.LaunchDescription([
        length_arg,
        Trace(
            session_name=f'trace-reference-system-{group_id}b',
